...
def TNBP_3_sat_interaction_new(G_fac,Nv,boundaries,clauses,tendencies,interactions,max_item,region_info,region_bound,device,cavity2=None):
    """
    ...
    if (cavity2!=None):
        cavity=cavity2
        bad_cavity_points = set()  # 存 (noden, node) 的 tuple
                        ...
                        if len(Ne_cavity)!=0:
                            if step > 0 :
                                ...
                                if cavity2 is not None:
                                    ...
                                    #print(f"[Step 0] Node {node} 的坏点：{bad_points_set}")
            ...
            for center_node in div:
                ...
                for node in neighborhood:
                    ...
                    if cavity2 is not None and (node, center_node) not in bad_cavity_points and flag:
                        ...
                        noskip2+=1
                    '''
                    is_inner_msg = inner_node(center_node, div_id, region_bound) and not inner_node(node, div_id,
                                                                                                    region_bound)
                    if (node in inner_msg) and is_inner_msg:
                        cavity[node][center_node] = inner_msg[node]
                        continue
                        ...
                        for interaction in interactions:
                            for adn in interaction:
                                if node == adn:
                                    for adc in interaction:
                                        if adc in Nv[
                                            center_node] and adc != adn and adc not in Ne_cavity and adc != center_node:
                                            Ne_cavity.append(adc)
                                            Ne_cavity = list(set(Ne_cavity))
                        if len(Ne_cavity) != 0:
                            if step > 0:
