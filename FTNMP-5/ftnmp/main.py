...
while True:
    try:
        run_mode_choice = int(input("请输入选项编号 (1-4): "))
        if 1 <= run_mode_choice <= 4:
...
for exp_idx in tqdm(range(NUM_EXPERIMENTS), desc="实验进度"):
    try:
        ...
        # ---------- 关键：每个 R 只调用一次 grn.get_regions_z，所有版本共享 ----------
        r_cache = {}
        for R_region in RRR:
            ...
            r_cache[R_region] = (devides, region_info, single_list, Nv, boundaries, devide_bound, degree_node)

        # 按顺序运行各版本，对每个 R 使用相同的区域划分
        for ver_name, module in versions_to_run:
            for R_region in RRR:
                devides, region_info, single_list, Nv, boundaries, devide_bound, degree_node = r_cache[R_region]
                ...
                if module == EFTO1:
                    ...
                    entropy_val, step_val, _ = module.EntropyZ_new(
                        G_fac, clauses, tendencies, devides, devide_bound, region_info,
                        single_list, max_item, Nv, boundaries, degree_node, device, None, cavity2
                    )
