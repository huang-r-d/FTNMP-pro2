...
def random_3_sat_graph(n_qubit, alpha):
    """
    Generate a random 4-SAT problem formula. (Keeping function name for compatibility)
    ...
    Returns:
    clauses (list of lists): A list of clauses, each clause is a list of 4 unique variable indices.
    tendencies (list of lists): A list of tendencies for each clause, with each tendency being a list of four integers (-1 or 1).
    """
    # Simply delegate to random_k_sat_graph with k=4
    return random_k_sat_graph(n_qubit, alpha, k=4)


def random_k_sat_graph(n_qubit, alpha, k):
    """
    Generate a random k-SAT problem formula.
...
def G_generator_fac(clauses):
    """
    Generate a graph representing the factor graph of a SAT problem.
...
def double_random_generate(n_glo, alpha_glo, n_cluster, alpha_range, min_cluster, max_cluster, seed):
    """
    Generate a random 4-SAT problem with multiple clusters, where each cluster has its own set of clauses and tendencies.
    ...
    Returns:
    clauses (list of lists): A list of clauses, each clause is a list of variable indices.
    tendencies (list of lists): A list of tendencies, with each tendency being a list of four integers (-1 or 1).
    """
    np.random.seed(seed)
    random.seed(seed)
    # Use k=4 for 4-SAT
    clauses, tendencies = random_k_sat_graph(n_glo, alpha_glo, k=4)
    ...
    for cs_id in range(len(cluster_sizes)):
        ...
        for ns in range(n_subclauses):
            # Generate 4-SAT clauses for the local dense cluster
            clause = random.sample(cluster, 4)
            if clause not in clauses:
                clauses.append(sorted(clause, reverse=True))
                tendency_cluster = np.random.randint(0, 2, 4)
