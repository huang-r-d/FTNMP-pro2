...
def information_of_fac(G_edges, clauses, tendencies, max_item):
    ...
    Returns:
        information(tc.Tensor): A tensor of shape (len(clauses), 8, len(G_edges)), where each entry represents the information of a factor node regarding its connections.
    """
    information = tc.zeros(len(clauses), 8, len(G_edges))  # Changed 6 -> 8 for 4-SAT
    ...
    for t in range(interation):
        ...
        for edge in G_edges:
            ...
            for nei_node in clausey:
                ...
                message_middle = tc.exp(information[fac_id, 2 * var_u_id:2 * var_u_id + 2,
                                        min(sliced):max(sliced) + 1] @ message_fac_to_var_ln[
                                                                       min(sliced):max(sliced) + 1].t())
            ...
            message_fac_to_var[message_fac_to_var_id] = (1 - damping) * message_new + damping * message_fac_to_var[
                message_fac_to_var_id]
